...
const { uploading = false, progress } = defineProps<{
  uploading?: boolean
...
<template>
  <label
    for="dropzone-file"
    class="border-border bg-surface flex aspect-square w-full flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-dashed p-2 text-center transition-colors"
    :class="uploading ? 'border-primary/50 pointer-events-none' : 'hover:border-primary/40 hover:bg-surface-2 cursor-pointer'"
    @dragover.prevent
    @drop.prevent="handleDrop">
    <!-- Uploading -->
    <template v-if="uploading">
      <LucideLoader2 class="text-primary mb-2 size-8 animate-spin" />
